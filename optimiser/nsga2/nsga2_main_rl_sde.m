function [pop, GD, SP, history] = nsga2_main_rl_sde(vel_profile)
...
% Fokus 2 Objektif: [Time, Energy]
    ...
    global var use_improved
    bounds = build_bounds_cccr(vel_profile, var, 5); % 5 km/h floor
    if ~isempty(use_improved) && logical(use_improved)
        if size(bounds,1) > dimension
            % Gradient-adaptive MID: remove 3rd DV for each var=3 section
            keep = true(size(bounds,1), 1);
            row = 0;
            for ii = 1:length(var)
                if var(ii) == 3
                    keep(row + 3) = false;
            ...
            bounds = bounds(keep, :);
        elseif dimension > size(bounds,1)
            n_extra = dimension - size(bounds,1);
            bounds  = [bounds; repmat([0.80, 1.00], n_extra, 1)];
        end
    end
    ...
    % Smart init: inject physics-informed seeds for improved CC_CR (Config D)
    if ~isempty(use_improved) && logical(use_improved)
        frac = 0.25;
...
function x = repair_cccr(x, bounds)
    ...
    %  var=3: HIGH >= LOW1 (gradient-adaptive MID active when use_improved=true)
    global var use_improved
    lb = bounds(:,1)'; ub = bounds(:,2)';
    x = min(max(x, lb), ub);
    ...
    for i = 1:numel(var)
        switch var(i)
            ...
            case 3
                ...
                x(k+1) = lo1;
                if ~isempty(use_improved) && logical(use_improved)
                    % Gradient-adaptive MID: var=3 has only 2 DVs (HIGH, LOW1)
                    k = k + 2;
                else
                    mid = x(k+2);
