function [allpop, GD, SP] = nsga2_main_improved_regular(vel_profile)
...
%     A dominates B if all(A<=B) && any(A<B)
    ...
    useVarBounds = false;
    try
        if ~isempty(var) && numel(var)==(size(vel_profile,1)-1) && sum(var)==dimension
            bounds = build_bounds_from_var(vel_profile, var, dimension);
            useVarBounds = true;
        end
    catch
        useVarBounds = false;
    ...
    if useVarBounds
        popX = repair_pop_from_var(popX, vel_profile, var);
    end
    ...
    for gen = 1:G
        ...
        if useVarBounds
            childX = repair_pop_from_var(childX, vel_profile, var);
        end
        ...
        scatter(xP(mP), yP(mP), 'filled');
            ...
            title(sprintf('NSGA-II gen %d', gen));
            drawnow;
        end
    end
end
...
% ===================== ECOAST bounds/repair (local subfunctions) =====================

function bounds = build_bounds_from_var(vel_profile, var, dimension)
%BUILD_BOUNDS_FROM_VAR Gene-wise bounds for CC_CR / E-COAST encodings.
% bounds is (dimension x 2): [lb ub] per gene (km/h).
%
% var per section:
%   1 -> [HIGH] (km/h)
%   2 -> [HIGH, LOW] (km/h)
%   3 -> [HIGH, LOW1, MID] (km/h)   (MID = early coasting trigger for E-COAST)
%
    if isempty(vel_profile) || size(vel_profile,2) < 2
        error('vel_profile invalid in build_bounds_from_var.');
    end
    if isempty(var)
        error('var is empty in build_bounds_from_var. Run decision_var_NO first.');
    end

    vlim = vel_profile(1:end-1,2);
    nSec = numel(var);

    if numel(vlim) ~= nSec
        error('Mismatch: length(var)=%d but vel_profile sections=%d.', nSec, numel(vlim));
    end
    if ~(sum(var) == dimension || sum(var)+2 == dimension)
        error('dimension mismatch: dimension=%d but sum(var)=%d (expected sum(var) or sum(var)+2).', dimension, sum(var));
    end

    bounds = zeros(dimension,2);
    k = 1;
    ...
    for i=1:nSec
        ...
        switch var(i)
            ...
            case 2
                % HIGH close to limit; LOW lower
                bounds(k,:)   = [max(0.35*v, vmin), v];      % HIGH
                bounds(k+1,:) = [max(0.05*v, 3),   v];       % LOW
                k = k + 2;

            case 3
                % E-COAST: MID should lie between LOW1 and HIGH
                bounds(k,:)   = [max(0.35*v, vmin), v];      % HIGH
                bounds(k+1,:) = [max(0.05*v, 3),   v];       % LOW1
                bounds(k+2,:) = [max(0.10*v, 5),   v];       % MID
                k = k + 3;

            otherwise
                error('Unknown var(%d)=%g. Expected 1/2/3.', i, var(i));
        end
    end
...
function popX = repair_pop_from_var(popX, vel_profile, var)
%REPAIR_POP_FROM_VAR Repair individuals so E-COAST genes actually matter.
% Ensures:
%   var==2: HIGH >= LOW, with minimum gap
%   var==3: HIGH >= MID >= LOW1, with minimum gaps
%
    if isempty(popX), return; end
    vlim = vel_profile(1:end-1,2);
    nSec = numel(var);
    ...
    for r = 1:size(popX,1)
        ...
        for i=1:nSec
            v = max(vlim(i), 5);
            gap    = max(2, 0.08*v);   % min HIGH-LOW window
            gapMid = max(1, 0.05*v);   % keep MID below HIGH a bit

            if var(i)==1
                X(k) = clamp_val(X(k), 0.15*v, v);
                k = k + 1;

            elseif var(i)==2
                HIGH = clamp_val(X(k),   0.20*v, v);
                LOW  = clamp_val(X(k+1), 0.02*v, v);

                if LOW > HIGH
                    tmp = HIGH; HIGH = LOW; LOW = tmp;
                end
                if (HIGH - LOW) < gap
                    LOW = max(0.02*v, HIGH - gap);
                end
                X(k)   = HIGH;
                X(k+1) = min(LOW, HIGH);
                k = k + 2;
            ...
            elseif var(i)==3
                HIGH = clamp_val(X(k),   0.20*v, v);
                LOW1 = clamp_val(X(k+1), 0.02*v, v);
                MID  = clamp_val(X(k+2), 0.05*v, v);

                if LOW1 > HIGH
                    tmp = HIGH; HIGH = LOW1; LOW1 = tmp;
                end
                if (HIGH - LOW1) < gap
                    LOW1 = max(0.02*v, HIGH - gap);
                end

                % MID between [LOW1, HIGH], not too close to HIGH
                MID = min(max(MID, LOW1 + 0.2*(HIGH-LOW1)), HIGH - gapMid);
                if MID <= LOW1
                    MID = LOW1 + 0.6*(HIGH-LOW1);
                end
                ...
                % fallback: clamp
                X(k) = clamp_val(X(k), 0, v);
                k = k + 1;
            end
        end
        % Optional accel scaling genes at the tail (alpha_trac, alpha_brake)
        if numel(X) == sum(var)+2
            X(end-1) = clamp_val(X(end-1), 0.60, 1.00);
            X(end)   = clamp_val(X(end)  , 0.60, 1.00);
        end
