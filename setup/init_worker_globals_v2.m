function init_worker_globals_v2(routePath, rollingstockPath, sim_in, use_improved_in)
%INIT_WORKER_GLOBALS_V2  Option 2: alpha_trac/alpha_brake fixed at 1.0 (not decision vars).
% Identical to init_worker_globals except dimension = sum(var) only — no alpha dims.
% Use this with run_E1_ablation_v2.m.
    ...
    decision_var_NO;
    % Option 2: alpha fixed at 1.0 — NOT a decision variable.
    % dimension = sum(var) only, no extra alpha dims.
    dimension = sum(var);

    fprintf('[WORKER INIT v2] use_improved=%d | strategy=%s | dim=%d | alpha=FIXED(1.0)\n', ...
