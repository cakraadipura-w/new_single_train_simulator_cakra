...
route_direction = 'up';
segment_catalog = get_guangzhou_line7_catalog(route_direction);
active_segment = segment_catalog(1);

%% 1. Load Rolling Stock & Set Global (seperti baseline)
fprintf('Loading rolling stock...\n');
run(fullfile(project_root, 'rollingstocks', 'rollingstock_Guangzhou_L7.m'));  % Semua parameter global terdefinisi
...
data = load(resolve_project_route_path(project_root, active_segment.file, active_segment.direction));
...
Target_T = active_segment.T_sched;  % detik (jadwal IS01)
