...
% - saves smoother plots and summaries under Dynamic_Programming/COPILOT/results
% =========================================================================
clear; clc; close all;
...
target_T_offsets = 0:4;  % default: [T_sched, T_sched+1, ...]
...
lambda_seed_count = 3;
auto_adjust_dv = true;
min_control_step_small_dx = 0.025;
...
improve.energy_polish_relative_span = 0.12;
improve.energy_polish_lambda_samples = 7;
...
for seg_run_idx = 1:numel(segments_to_run)
	...
	segment_summary_file = fullfile(results_output_dir, sprintf('%s_copilot_summary.txt', route_base_name));
	segment_results_file = fullfile(results_output_dir, sprintf('%s_copilot_results.mat', route_base_name));
...
aggregate_summary_file = fullfile(results_output_dir, 'all_segments_copilot_summary.txt');
aggregate_results_file = fullfile(results_output_dir, 'all_segments_copilot_results.mat');
...
fprintf('COPILOT results saved in: %s\n', results_output_dir);
...
function summary_text = build_segment_summary_text(active_segment, target_T_values, target_results, total_search_time_s, run_meta)
	lines = {
		sprintf('=== COPILOT TARGET-TIME SUMMARY | %s ===', active_segment.name)
...
function summary_text = build_overall_summary_text(segment_runs, overall_batch_time_s, results_output_dir)
	lines = {
		'=== COPILOT SUMMARY FOR ALL SEGMENTS ==='
...
function plot_files = save_segment_plots(active_segment, vel_lim, target_results, display_cfg, results_output_dir)
	route_base_name = erase(active_segment.file, '.mat');
	speed_plot_file = fullfile(results_output_dir, sprintf('%s_copilot_speed_profile.png', route_base_name));
	energy_plot_file = fullfile(results_output_dir, sprintf('%s_copilot_energy_curve.png', route_base_name));
	...
	speed_fig = figure('Name', sprintf('COPILOT %s speed profile', active_segment.name), ...
	...
	title(sprintf('COPILOT Smoothed DP Speed Profile | %s', active_segment.name));
	grid on;
	...
	energy_fig = figure('Name', sprintf('COPILOT %s energy curve', active_segment.name), ...
		'Visible', figure_visibility, 'Color', 'w');
	hold on;
	...
	title(sprintf('COPILOT Energy vs Target Time | %s', active_segment.name));
	grid on;
