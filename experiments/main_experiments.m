...
%% =========================================================================
ALL_SEGMENTS = get_guangzhou_line7_catalog(ROUTE_DIRECTION, [130, 170, 185, 180, 185, 220, 210, 330]);
...
for seg_k = 1:numel(SEGMENTS_TO_RUN)
    ...
    ACTIVE_ROUTE_DIRECTION = ACTIVE_SEG.direction;
    ACTIVE_RS  = RS_FILE;
    ACTIVE_NRUNS = N_RUNS;

    % Output folder per-segment
    OUT_DIR = fullfile(project_root, 'experiment_results', seg_name);
    ...
    if RUN_BASELINE
        fprintf('--- BASELINE: %s ---\n', seg_name);
        t0_bl = tic;
        rp = resolve_project_route_path(project_root, ACTIVE_SEG.file, ACTIVE_SEG.direction);
        if isempty(rp)
