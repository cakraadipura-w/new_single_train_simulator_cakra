%% summarize_e1_schedule_energy.m
% Run this file directly from the MATLAB editor.
%
% Summarizes the E1 Pareto fronts under experiment_results/ISxx by picking
...
for seg_idx = 1:numel(catalog)
    seg = catalog(seg_idx);
    [result_file, experiment_id] = resolve_experiment_file(script_dir, seg.name);

    if isempty(result_file)
        summary_rows(end + 1, 1) = build_missing_summary_row(seg, '', 'missing E1/E5 data'); %#ok<AGROW>
        continue;
    end
    ...
    for cfg_idx = 1:numel(config_ids)
        ...
        row.experiment_id = string(experiment_id);
    ...
    if isempty(segment_config_rows)
        summary_rows(end + 1, 1) = build_missing_summary_row(seg, result_file, 'empty compatible results'); %#ok<AGROW>
        continue;
    end
    ...
    summary.experiment_id = string(experiment_id);
...
summary_csv = fullfile(script_dir, 'e1_schedule_energy_summary.csv');
config_csv = fullfile(script_dir, 'e1_schedule_energy_by_config.csv');
summary_txt = fullfile(script_dir, 'e1_schedule_energy_summary.txt');
...
function [result_file, experiment_id] = resolve_experiment_file(script_dir, segment_name)
    candidates = { ...
        {'E1', fullfile(script_dir, segment_name, 'E1_all_configs.mat')}; ...
        {'E5', fullfile(script_dir, segment_name, 'E5_all_configs.mat')}};

    result_file = '';
    experiment_id = '';
    for idx = 1:numel(candidates)
        candidate = candidates{idx};
        if isfile(candidate{2})
            experiment_id = candidate{1};
            result_file = candidate{2};
            return;
        end
    end
end
...
function write_summary_text(file_path, summary_table, config_table, target_tolerance_s)
    ...
    fprintf(fid, 'Experiment schedule-energy summary\n');
