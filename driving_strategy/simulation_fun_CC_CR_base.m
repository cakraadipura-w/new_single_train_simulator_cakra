...
% accrate_fixset(:)=1.2;
% accrate_fixset(:)=0.95;
accrate_fixset(:)=max_accel_trac;
% brakerate_fixset(:)=2.0;
% brakerate_fixset(:)=0.57;
brakerate_fixset(:)=max_accel_brake;
kdset(:)=0.5;
train_controlset(:)=1;
...
for i=1:(Size-1)
    ...
    if var(SPDLi)==3||var(SPDLi)==1 %coasting-remotoring
        ...
        if (velF(1,i)<vel_limit(1,i+1) && velF(1,i)<HIGH && coast_start_flag==0)||(velF(1,i)<LOW1) %traction
            ...
            if velF(1,i+1)>=HIGH && HIGH~=LOW1  %start coasting from coasting up bound
                coast_start_flag=1;
            end
        else
